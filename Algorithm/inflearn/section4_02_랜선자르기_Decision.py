...
def Count(len):  # n개의 랜선 1개의 길이
    cnt = 0
    for x in Line:
        cnt += (x // len)  # k개의 랜선 하나하나 길이가 x, len으로 나눈다  # 802 // 182 -> 4
    return cnt
...
for i in range(k):
    tmp = int(input())
    Line.append(tmp)
    largest = max(largest, tmp)  # 기존의 값과 새로운 값을 비교해서 큰 값을 largest에 갱신

# 이분 검색 시작
lt = 1
rt = largest
while lt <= rt:
    mid = (lt+rt) // 2  # mid는 n개의 랜선을 만들 때 1개의 랜선의 길이
    if Count(mid) >= n:
        res = mid
        lt = mid + 1
    else:  # 개수가 부족, 답이 되지 않는다.
        rt = mid - 1
print(res)

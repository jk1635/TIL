...
res = set()
for i in range(n):  # index 0 부터 돌아야한다. 0부터 입력됐기 때문.
    for j in range(i+1, n):  # j는 i바로 뒤부터 돌아야한다.
        for m in range(j+1, n):
            res.add(a[i]+a[j]+a[m])  # set자료구조는 append가 아니라 add

res = list(res)  # set은 sort가 없다.
